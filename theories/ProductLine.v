...
(* ================================================================== *)
(* Tactic: enumerate valid variants and discharge family_wide          *)
(* ================================================================== *)

(** Generate all subsets of a list (power set). *)
Fixpoint power_set (features : list Feature) : list Variant :=
  match features with
  | [] => [[]]
  | f :: rest =>
    let subs := power_set rest in
    subs ++ map (cons f) subs
  end.

(** All valid variants of a feature model (finite enumeration). *)
Definition enumerate_valid_variants (fm : FeatureModel)
    : list Variant :=
  filter (valid_variant fm) (power_set fm.(fm_features)).

(** Check [structural_checks] on every valid variant. *)
Definition check_all_variants_structural (plc : ProductLineCase)
    : bool :=
  forallb (fun v => structural_checks (project_variant plc v))
          (enumerate_valid_variants plc.(plc_fm)).

(** Tactic: discharge the structural premise of [family_wide_wf_check]
    by enumerating all valid variants of a finite [FeatureModel] and
    running [vm_compute] on each.  Only works when [fm_features] is
    a concrete list (finite enumeration). *)
Ltac decide_family_wide_structural :=
  match goal with
  | |- forall v, valid_variant ?fm v = true ->
       check_variant_wf ?plc v = true =>
    let v := fresh "v" in
    let Hv := fresh "Hv" in
    intros v Hv;
