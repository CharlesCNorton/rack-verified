  ...
  let rec gen depth =
    ...
    if depth <= 0 then begin
      nodes := { node_id = coq_of_string id; node_kind = Solution;
        ...
        node_evidence = Some (ProofTerm (coq_of_string "p", Some (fun _ -> True)));
...
let () =
  ...
  for i = 0 to 49 do
    ...
    | _ ->
      ...
      let new_ev = ProofTerm (coq_of_string "updated", Some (fun _ -> True)) in
  ...
  for i = 0 to 49 do
    ...
    let wf_before = check_wf_extended ac |> coq_bool_to_bool in
    let wf_after = check_wf_extended ac' |> coq_bool_to_bool in
    check (Printf.sprintf "delta_empty_%d" i) (wf_before = wf_after)
  done;

  (* Delta: RemoveNode on sole-child nodes breaks structure *)
  Printf.printf "[Delta: RemoveNode sole-child breaks structure]\n";
  for i = 0 to 49 do
    let ac = gen_well_formed_tree (2 + rand_int 2) in
    let nodes = list_of_coq ac.ac_nodes in
    let links = list_of_coq ac.ac_links in
    (* Find nodes that are the sole SupportedBy child of their parent *)
    let sole_children = List.filter (fun n ->
      let id_s = string_of_coq n.node_id in
      let parents = List.filter (fun l ->
        l.link_kind = SupportedBy &&
        string_of_coq l.link_to = id_s) links in
      match parents with
      | [p] ->
        let siblings = List.filter (fun l ->
          l.link_kind = SupportedBy &&
          string_of_coq l.link_from = string_of_coq p.link_from) links in
        List.length siblings = 1
      | _ -> false) nodes in
    match sole_children with
    | [] -> check (Printf.sprintf "delta_rm_%d" i) true
    | _ ->
      let target = List.nth sole_children (rand_int (List.length sole_children)) in
      ...
      check (Printf.sprintf "delta_rm_%d" i)
        (not (check_wf_extended ac' |> coq_bool_to_bool))
  done;
